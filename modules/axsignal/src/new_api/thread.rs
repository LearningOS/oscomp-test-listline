...
pub struct ThreadSignalManager<M> {
    /// The process-level signal manager
    proc: Arc<ProcessSignalManager<M>>,
    /// The pending signals
...
impl<M: RawMutex> ThreadSignalManager<M> {
    pub fn new(proc: Arc<ProcessSignalManager<M>>) -> Self {
        Self {
            proc,
